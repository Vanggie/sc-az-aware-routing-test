...
delete_services() {
    ...
    aws ecs --region $REGION --endpoint $ECS_ENDPOINT update-service \
        ...
        --service az-aware-fe-service \
        --desired-count 0 2>/dev/null || true
    
    aws ecs --region $REGION --endpoint $ECS_ENDPOINT update-service \
        --cluster $CLUSTER_NAME \
        --service az-aware-backend-service \
        --desired-count 0 2>/dev/null || true
    ...
    aws ecs --region $REGION --endpoint $ECS_ENDPOINT delete-service \
        --cluster $CLUSTER_NAME \
        --service az-aware-fe-service 2>/dev/null || true
    
    aws ecs --region $REGION --endpoint $ECS_ENDPOINT delete-service \
        --cluster $CLUSTER_NAME \
        --service az-aware-backend-service 2>/dev/null || true
...
deregister_task_definitions() {
    ...
    for family in az-aware-fe-service az-aware-backend-service; do
...
main() {
    if [ -z "$1" ]; then
        ...
        echo "Scope: ecs-services (services/tasks), ecs-all (services/cluster/tasks), or all (includes lb and security groups)"
        exit 1
    fi
